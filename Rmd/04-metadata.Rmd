...
Station locations (see counts in Table \@ref(tab:tbl-num-stns)):
...
if(params$retrieve_fr_db) {
  ...
  dt.stns[, `:=`(lat = lat + rnorm(nrow(dt.stns)) * 0.03,
                 lon = lon + rnorm(nrow(dt.stns)) * 0.03)]
...
leaflet(elementId = "aq_stations") %>% addTiles() %>% addCircles(data = sf.stns)
```
...
DT::datatable(
  ...
  caption = "Temporal coverage by state and variable",
  elementId = "tbl_temporal_coverage_state_variable"
)
...
The National Air Pollution Monitoring Database converts all hourly data to be in 0-23 hour-beginning format (i.e. 0 represents midnight to 1am on the date specified). For different states and territories the source data can vary.
